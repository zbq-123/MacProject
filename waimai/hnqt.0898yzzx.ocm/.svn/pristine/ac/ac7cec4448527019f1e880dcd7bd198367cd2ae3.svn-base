...
class SupermarketGood extends Model
{
    ...
    public function getListData($maps, $searchFields, $limit, $page){
        ...
        if(!empty($result)){
            foreach($result as $k=>$v){
                $res=SupermarketSort::where('id',$v['category_id'])->field('id,name')->find();
                if(!empty($res)){
        ...
        if(!empty($result)){
            foreach($result as $k=>$v){
                $supermarket_res=Supermarket::where('id',$v['supermarket_id'])->field('id,name')->find();
                if(!empty($res)){
