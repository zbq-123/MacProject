...
;!function(){
  ...
  */
  ...
  Class.prototype.init = function(){
    var that = this
    ,options = that.config
    ,dateType = 'yyyy|y|MM|M|dd|d|HH|H|mm|m|ss|s|hh|h'
  ...
  Class.prototype.checkDate = function(fn){
    ...
    ,value = (that.isInput(elem) ? elem.value : (options.position === 'static' ? '' : elem.innerHTML)).replace('上', "1").replace('下', "2")
  ...
  Class.prototype.parse = function(state, date){
    ...
    lay.each(format, function(i, item){
      ...
      } else if(/hh|h/.test(item)){ //半年
        format[i] = lay.digit(dateTime.month + 1, item.length).replace('1', "上").replace('2', "下");
      }
    });
