...
use app\admin\model\DigitalCategory as DigitalCategory_mod;
use app\common\controller\AdminBase;
use think\Db;
use util\Ret;
use app\admin\model\Digital;

class DigitalCategory extends AdminBase
{
    public function index()
    {
        if (request()->isAjax()) {
            $campus = new DigitalCategory_mod();
    ...
    public function delete(){
        $subject =DigitalCategory_mod::where(['id'=>input('post.id')])->find();
        if (false == $subject) {
            abort(608, '资源不存在');
        }
        $subject->deleted = 1;
    ...
    public function edit(){
        if (request()->isPost()){
            ...
            $column = new DigitalCategory_mod();
            ...
            $id = request()->param('id');
            $friendship=DigitalCategory_mod::where('id', $id)->find();
            if (empty($friendship)) {
                $this->error("资源不存在");
            }
