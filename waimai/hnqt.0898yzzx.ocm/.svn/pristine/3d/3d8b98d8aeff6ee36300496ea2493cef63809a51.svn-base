...
class Orders extends Collection{
    ...
    public function wxpay_goods(){
        $store_id = input('post.store_id/d')?:17;
        $buy_goods = input('post.buy_goods/s')?:"327:94,3231:,3241:104,327:95";
        $buy_number = input('post.buy_number/s')?:'2,1,2,1';
        $address_id = input('post.address_id/d')?:244;
        $remake = input('post.remake');
        $use_coupon = input('post.use_coupon'); //rice 优惠券
        $use_month_card = input('post.use_month_card'); //rice 月卡
        $user_id = input('post.user_id/d')?:1727;
        $type = input('post.type/d');//商品类型1外卖2数码
        //$sku_id = input('post.sku_id');//sku商品id
...
//            }
            ...
            if(!empty($goods[1])){
                $sku= Db::name('spec_goods_price')
                    ->where('id',$goods[1])
                    ->where('goods_id',$goods[0])
                    ->field('id,price')
                    ->find();
                if(!empty($sku)){
                    $buy_info[$key]['price']    =$sku['price']*100;
                }
            }else{
               // 商品是否促销
                $maps['start_time'] = ['<',date('Y-m-d H:i:s',time())];
        ...
        if(1==2){//支付宝
            ...
            if (isset($arr['prepay_id'])) {
                ...
                foreach ($goods_id as $key => $value) {
                    // 查库存
                    if(!empty($value[1])){
                        $get_goods_info = Db::name('spec_goods_price')->where('goods_id',$value[1])->field('store_count')->find();
                        $up_stocks = $get_goods_info['store_count'] - $buy_number[$key];
                        Db::name('spec_goods_price')->where('goods_id',$value[1])->update(['store_count' => $up_stocks]);
                    }else{
                        $up_goods_info = Db::name('goods')->where('id',$value[0])->field('stock')->find();
                        $up_stock = $up_goods_info['stock'] - $buy_number[$key];
                        Db::name('goods')->where('id',$value[0])->update(['stock' => $up_stock]);
                    }
...
//                }
  ...
  * */
    public function wxpay_goods_orders()
    {
        ...
        if(!empty($orders_data['order_name'])){
           if(strstr($orders_data['order_name'],'数码产品')){
              
               $store = Db::name('digital')
                   ->where('id',$orders_data['store_id'])
                   ->where('deleted',0)
                   ->field("id store_id,name,phone,address,campus_id")
                   ->find();
               if(empty($store)){
                   return json(['code'=>204,'msg'=>'店铺不存在']);
               }
               ...
               foreach ($goods_ids as $key=>$goods) {
                   $goods_info = Db::name('digital_good')
                       ->where('id', $goods)
                       ->where('status', 1)
                       ->where('deleted', 0)
...
//                }
//            }
            if(strstr($orderinfo['order_name'],'数码产品')){
                $surface1='digitalspec_goods_price';
                $surface   ='digital';
                $surface2='digital_good';
            }else{
                $surface1='spec_goods_price';
                $surface   ='store';
                $surface2='goods';
            }
            $buy_goods = explode(",",$orderinfo['goods_ids']);
            foreach($buy_goods as $key =>$value){
            ...
            foreach($order_goods_detail as $k =>$v){
                if($goods_id[$k][0]==$v[0]){
                    $skudata=Db::name($surface1)->where('id',$goods_id[$k][1])->find();
                    if(!empty($skudata)){
                        $goods[$k]['sku']=$skudata['key1'].'-'.$skudata['key2'].'-'.$skudata['key3'].'-'.$skudata['key4'];
                        $goods[$k]['sku_id']=$goods_id[$k][1];
                    }
                }
                $goods[$k]['id']=$v[0];
                $goods[$k]['name']=$v[2];
            ...
            $res_phone=Db::name($surface)->where('id',$orderinfo['store_id'])->field('phone')->find();
            if(!empty($res_phone)){
                $orderinfo['store_phone']=$res_phone['phone'];
            }else{
    ...
    public function apply_refund()
    {
        ...
        if (!$validate->check(input('post.'))) {
            return json(['code'=>202,'msg'=>'参数错误，请重试']);
        }
        ...
        if(empty($orders)){
            return json(['code'=>203,'msg'=>'该订单暂不支持申请退款']);
        }

        if($orders['user_deleted'] == 1){
            return json(['code'=>205,'msg'=>'操作失败，该订单已被删除']);
        }
        ...
        if($cancel_res){
            ...
            return json(['code'=>200,'msg'=>'申请退款已提交，请耐心等待','data'=>$cancel_res]);
    ...
    public function del_orders(){
        ...
        $res=Db::name('orders')->where('order_number',$order_number)->where('user_id',$user_id)->field('id')->find();

        if(empty($res)){
            return json(['code'=>202,'msg'=>'查询不到该订单!']);
        }
        $del_res=Db::name('orders')->where('id',$res['id'])->update(['deleted'=>1]);
        if(!empty($del_res)){
            return json(['code'=>200,'msg'=>'删除成功','data'=>$del_res]);
        }else{
            return json(['code'=>204,'msg'=>'删除失败']);
        }
    }

    //数码商品用户确认订单，生成订单信息，返回支付信息
    public function wxpay_digital_good(){
        $store_id = input('post.store_id/d')?:17;
        $buy_goods = input('post.buy_goods/s')?:"327:94,3231:,3241:104,327:95";
        $buy_number = input('post.buy_number/s')?:'2,1,2,1';
        $address_id = input('post.address_id/d')?:244;
        ...
        if(empty($address)){
            return json(['code'=>202,'msg'=>'地址不可用']);
        }
        //2 获取下单商品信息
        $store = Db::name('digital')
            ->where('id',$store_id)
            ->where('deleted',0)
            ->field("id store_id,name,phone,address,image,campus_id")
            ->find();
        if(empty($store)){
            return json(['code'=>203,'msg'=>'商家不存在']);
        }
        $campus = Db::name('campus')
            ->where('id',$store['campus_id'])
            ->where('deleted',0)
        ...
        if(empty($campus)){
            return json(['code'=>205,'msg'=>'该校区暂不支持下单']);
        }
        //所属校区
        $store['campus_name'] = $campus['name'];
        // 获取购买商品信息 计算金额
        ...
        foreach ($goods_id as $key=>$goods){
            if($buy_number[$key] <= 0){
                return json(['code'=>206,'msg'=>'商品异常，请重试']);
            }
            $buy_info[$key] = Db::name('digital_good')
                ->where('id',$goods[0])
                ->where('status',1)
                ->where('deleted',0)
                ->field('id goods_id,image,name,price,unit')
                ->find();
            if(empty($buy_info[$key])){
                return json(['code'=>207,'msg'=>'部分商品已下架']);
            }
            $buy_info[$key]['price']=$buy_info[$key]['price']*100;
            //是否是sku商品
            if(!empty($goods[1])){
                $sku= Db::name('digitalspec_goods_price')
                    ->where('id',$goods[1])
                    ->where('goods_id',$goods[0])
                    ->field('id,price')
                    ->find();
            ...
            $goods_count += $buy_number[$key];
            $goods_detail .= '--onelist--'.$buy_info[$key]['goods_id'];     //1 商品id
            $goods_detail .= '--twolist--'.$buy_info[$key]['goods_id'];       //2 商品编号
            $goods_detail .= '--twolist--'.$buy_info[$key]['name'];         //3 商品名称
            $goods_detail .= '--twolist--'.$buy_number[$key];               //4 购买数量
            $goods_detail .= '--twolist--'.$buy_info[$key]['price'];        //5 单价
            $goods_detail .= '--twolist--'.$buy_info[$key]['unit'];         //6 单位
        ...
        $orders_data['address_id'] = $address['address_id'];
        $orders_data['count'] = $goods_count;
        $orders_data['box_type'] = '';
        $orders_data['box_price'] = '';
        $orders_data['box_name'] = '';
        $orders_data['manage_ratio'] = '';
        $orders_data['manage_price'] = '';
        $orders_data['develop_ratio'] = '';
        $orders_data['develop_price'] = '';
        $orders_data['pay_manage_ratio'] = '';
        $orders_data['pay_manage_price'] = '';
        // $orders_data['store_price'] = $all_price - $orders_data['manage_price'] - $orders_data['develop_price'] - $orders_data['pay_manage_price'];

        // 计算订单店铺获得金额
        //$store_price = $all_price - $orders_data['manage_price'] - $orders_data['develop_price'] - $orders_data['pay_manage_price'];
       
        $orders_data['store_price'] = $all_price;
        $orders_data['convey_price'] = 10;
        $orders_data['convey_name'] = '运费';
        $orders_data['total_price'] = $all_price;
        $orders_data['campus_id'] = $store['campus_id'];
        $orders_data['campus_name'] = $store['campus_name'];
        $orders_data['delivery_name'] = $address['delivery_name'];
        ...
        $orders_data['goods_detail'] = $goods_detail;
        $orders_data['order_source'] = 2;
        $orders_data['order_name'] = '数码产品-'.$store['name'];
        $orders_data['order_describe'] = $buy_info[0]['name'].'等，共'.$orders_data['count'].'件商品';
        $orders_data['create_time'] = $create_time;
        $orders_data['use_month_card'] = '';
        $orders_data['use_coupon'] = '';
        $orders_data['discount_money'] = '';
        $orders_data['remake'] = $remake;
        /*订单数据生成完成，准备插入数据*/
        // 启动事务
        Db::startTrans();
        ...
        if($is_true){
            ...
            Db::rollback();
            return json(['code'=>208,'msg'=>'下单失败,请重试']);
        }
        //调用支付方式 RICE
        if(1==2){//支付宝
            $wappay = new Wappay();
...
//                }
                ...
                foreach ($goods_id as $key => $value) {
                    // 查库存
                    if(!empty($value[1])){
                        $get_goods_info = Db::name('digitalspec_goods_price')->where('goods_id',$value[1])->field('store_count')->find();
                        $up_stocks = $get_goods_info['store_count'] - $buy_number[$key];
                        Db::name('digitalspec_goods_price')->where('goods_id',$value[1])->update(['store_count' => $up_stocks]);
                    }else{
                        $up_goods_info = Db::name('digital_good')->where('id',$value[0])->field('stock')->find();
                        $up_stock = $up_goods_info['stock'] - $buy_number[$key];
                        Db::name('digital_good')->where('id',$value[0])->update(['stock' => $up_stock]);
                    }
                }
                return json(['code'=>200,'msg'=>'操作成功','data'=>$data]);
            } else {
