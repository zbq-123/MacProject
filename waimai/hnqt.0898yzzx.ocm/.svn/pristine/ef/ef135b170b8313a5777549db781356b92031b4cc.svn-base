...
class Campus extends Model
{
    ...
    public function getCampusName($map)
    {
       /* if (session('admin.Portal_id') != null) {
            $map['id'] = session('admin.Portal_id');
        }*/
        $result = Campus::scope('campusName')->where($map)->select();
        $count = Campus::where($map)->count();
    ...
    public function getListData($maps, $searchFields, $limit, $page){
        ...
        $result = $this::scope('StoreList')->with('admin')->where($maps)->limit($limit)->page($page)->order('sort desc,create_time desc')->select();
        $count = $this->where($maps)->count();

        return new listData($result, $count);
    }

    public function editCampus($data){
