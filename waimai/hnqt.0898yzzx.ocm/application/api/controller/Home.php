...
class Home extends Collection{
    ...
    public function get_phone(){
        ...
        if(empty($code)||empty($username)||empty($userimage)){
        ...
        $datas = json_decode(curl_exec($curl), true);
        curl_close($curl);
        if(!empty($datas['openid'])){
            $openid = $datas['openid'];
            $user_info =  Db::name('user')
                ->where('gz_openid',$openid)
                ->field('id as user_id,nickname,image')
                ->find();
                if($username!=$user_info['nickname']){
                    $res=Db::name('user')->where('id',$user_info['user_id'])->update(['nickname'=>$username]);
                    if(!empty($res)){
                        $user_info['nickname']=$username;
                    }
                }
                if($userimage!=$user_info['image']){
                    $result=Db::name('user')->where('id',$user_info['user_id'])->update(['image'=>$userimage]);
                     if(!empty($result)){
    ...
    public function get_phone1(){
        //获取到的code
        $phoneCode = input('code')?:'b8a06cad3858052546241718d4a8176e25abb5993ebe40cff15d09a7ad46a486';
        if(empty($phoneCode)){
            return json(['code'=>201,'msg'=>'参数有误']);
        }
...
//            return json(['code'=>200,'msg'=>'获取成功','data'=>$user_info['phone']]);
//        }
...
//        curl_setopt($getNumber, CURLOPT_CONNECTTIMEOUT, 10);
        ...
        curl_setopt($getNumber, CURLOPT_POST, 1);
        curl_setopt($getNumber, CURLOPT_POSTFIELDS, $finalCode);
        curl_setopt($getNumber, CURLOPT_RETURNTRANSFER, 1);//此处若设置为0则为直接输出,设置为1则为将返回结果赋值给变量
        curl_setopt($getNumber, CURLOPT_CONNECTTIMEOUT, 10);
