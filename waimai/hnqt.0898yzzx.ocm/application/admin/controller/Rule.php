...
class Rule extends AdminBase
{
    ...
    {
        if (request()->isAjax()) {
            $admin = Admin::with('group')->field("id, login_name, image, real_name, create_time, last_login_time, disabled, is_root,store_id,digital_id")->select();
            ...
            $digital=Db::name('digital')->where('deleted', 0)->order('update_time desc')->select();
            $this->assign('digital', $digital);
            return $this->fetch();
        }
    }
    //修改添加管理员
