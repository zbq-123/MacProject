...
class SupermarketSort extends Model
{
    ...
    public function getCategory($map)
    {
        ...
        $result = SupermarketSort::scope('categoryName')->where($map)->select();
        $count = SupermarketSort::where($map)->count();
    ...
    public function getListData($maps, $searchFields, $limit, $page){
        ...
        if(!empty($result)){
            foreach($result as $k =>$v){
                $res=Db::name('supermarket')->where('id',$v['supermarket_id'])->find();
                if($res){
