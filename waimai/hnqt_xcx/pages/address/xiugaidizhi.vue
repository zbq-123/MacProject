<template>
	<view>
		<u-navbar title="修改地址" :autoBack="true" bgColor="transparent" leftIcon="arrow-left" :fixed="true"
			...
			<ul class='u0'>
				<li>
					<view style='display: flex;align-items: center; font-size: 30rpx;'>
						...
						<input type="text" placeholder="姓名" v-model="delivery_name">
					</view>
				</li>
				<li>
				...
				<li>
					<view style='display: flex;align-items: center;font-size: 30rpx;'>
						...
						<input type="number" placeholder="手机号码" v-model="delivery_phone">
					</view>
				</li>
				<li>
					<view style='display: flex;align-items: center;font-size: 30rpx;'>
						<text class="left">地址</text>
						<input type="text" placeholder="收货地址:包含门牌号" v-model="delivery_address" />
					</view>
				</li>
				<li>
					<view style='display: flex;align-items: center;font-size: 30rpx;text-align: center;'>
		...
		<view style="position: relative;">
			<view class="vie5" :style="{display: `${displayStatic}`}" @click="baocun">
				<button>保存</button>
			</view>
...
<script>
	export default {
		...
		onLoad(options) {
			...
			this.delivery_name = options.delivery_name
			this.delivery_phone = options.delivery_phone
		...
		methods: {
			baocun() {
				...
				uni.$u.http.post('api/users/edit_address', {
