<template>
	<view>
		...
		<navigator class="goods" url="../address/wodedizhi">
			<view style="display: flex;align-items: center;justify-content: space-between;">
				<view style="display: flex;flex-flow: column;">
					...
					<text
						style="font-size: 25rpx;margin-top: 10rpx;color: gray;">{{address_name}}<block v-if="gender!=''">{{gender==0?'男士':'女士'}}</block>{{address_phone}}</text>
				</view>
		...
		<view class="goods">
			...
			<!-- 月卡 -->
			<!-- <view style="padding: 30rpx;" v-if="couponList!=''">
				<view
					style="font-weight: bold;font-size: 28rpx;color:#333333;display: flex;justify-content: space-between;">
					月卡<view style="color:#333333;" @click="show = true">请选择</view>
				</view>
		...
		<view class="card" @click="buyCard">
			<div class="card_submit">
				...
				<view class="p">
					<text>点击购买</text>
					<text style="font-size: 10px;color: azure;">大额优惠劵、不限各种美食等</text>
				</view>
			</div>
		</view>
		<view class="pay">
...
<script>
	export default {
		...
		onLoad(options) {
			...
			this.manyprice = this.zongshu = uni.getStorageSync('total') + JSON.parse(Object.values(options))[0]
				.delivery_price
