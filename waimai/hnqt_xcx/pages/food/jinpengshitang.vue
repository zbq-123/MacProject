	...
	<view>
		...
		<view v-show="Inv == 0">
			...
			<scroll-view scroll-y="true" :scroll-with-animation="true" :scroll-into-view="clickToId" class="left"
				:style="[{height:windowHeight+'px',top:windowtop + 'px'}]">
			...
			<scroll-view class="right" :scroll-into-view="clickId" @scroll="scroll" :scroll-with-animation="true"
				:scroll-y="true" :style="[{height:windowHeight+'px',top:windowtop + 'px'}]" :scroll-top="scrolTop">
				<!-- 要做v-for循环-->
				<view v-for="(xiangmu,index) in shop" :key="index" class="rightMenu">
					<view class="right_title" :id="'po'+index">
						{{xiangmu.name}}
					</view>
					<view class="right_content" :id="'go'+e" v-for="(c,e) in shop_details" :key="e"
						v-if="xiangmu.category_id == c.goods_category_id">
						<image :src="url1+c.image" style="width: 146rpx;height: 146rpx;"></image>
		...
		<view class="current_l" v-show="Inv == 1">
			<scroll-view class="vie6" scroll-y="true" :style="[{height:windowHeight+'px',top:windowtop + 'px'}]">
		...
		<view class="bottom">
			<view class="vie5" :style="{display: `${sjStatic}`}">
				<view style="position: relative;top: 0;left: -24rpx;width: 23%;" id="cart">
					<image :src='gouwuImg' style="width: 240rpx;height: 240rpx;" @click="dianjigouwuImg">
					</image>
				...
				<view style="display: flex;align-items: center;width: 77%;justify-content: flex-start;width: 80%">
					<view
						...
						{{totalPrice==shanjia.box_price?'￥0':totalPrice!=undefined?'￥'+totalPrice:'￥...'}}
					</view>
					<view style="color: #FFFFFF;font-size: 24rpx;">
						另需配送费￥{{shanjia.delivery_price!=undefined?shanjia.delivery_price:'...'}}
					</view>
				</view>
...
<script>
	export default {
					...
					this.$nextTick(() => {
						setTimeout(() => {
							for (var i = 0; i < shop_details.length; i++) {
								...
								uni.createSelectorQuery().select('#go' + i).boundingClientRect(data => {
									this.goList.push(data.top - this.windowtop)
								}).exec();
							}
						}, 1000)
					})
						...
						shop_details.forEach(el => {
							goods_id.forEach(el2 => {
								...
								if (el.goods_id == data[0]) {
									el.goods_num = 1; //数量
									el.isselect = true; //选中
									this.total += (el.salesprice != '' ? el.salesprice : el
										.price) * el.goods_num //此处修改页面的价格
									el.total += (el.salesprice != '' ? el.salesprice : el.price) *
										el.goods_num //此处修改自定义商品的价格
									...
									this.gouwucheAdd.push({
										...
										image: el.image,
										name: el.name,
										price: el.salesprice != '' ? el.salesprice : el
											.price,
		...
		methods: {
			...
			submit() {
				const value = [];
				this.shop_details.forEach((item,index) => {
					if (item.isrequired == 1 && item.isselect == false) {
						...
						uni.createSelectorQuery().select('#go'+index).boundingClientRect(
							data => { //目标位置的节点，类class或者id
								this.goList.forEach((i,e)=>{
									if(e==index){
										this.scrolTop = i
									}
								})
