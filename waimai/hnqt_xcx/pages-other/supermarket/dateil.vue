	...
	<view>
		...
		<view id="store" :style="[{top:storetop}]">
			<view style="display: flex;">
				<image :src="url1+shanjia.logo" mode=""></image>
		...
		<view v-show="Inv == 0">
			...
			<scroll-view scroll-y="true" :scroll-with-animation="true" :scroll-into-view="clickToId" class="left"
				...
				<view v-for="(abc,index) in shop" :key="index">
					<view :class="['vie8',{active:index===currentNum}]" :id="'to'+index" @click="setId(index)">
						<view class="title">{{abc.name}}</view>
						<view class="vie10" :style="{display:`${dabaofeiStatic}`}">{{dabaofeijia}}</view>
					</view>
				</view>
			</scroll-view>
			<!-- 右侧商品内容 -->
			<scroll-view class="right" :scroll-into-view="clickId" @scroll="scroll" :scroll-with-animation="true"
				...
				<view v-for="(xiangmu,index) in shop" :key="index">
					...
					<view class="right_content" v-for="(c,e) in shop_details" :key="e"
						v-if="xiangmu.category_id == c.goods_category_id">
						...
						<view style="margin-left: 15rpx;width: 62%;">
							...
							<view class="right_number" v-show="shanjia.status!=2">
								<text style="font-size: 16px;" v-if="!c.salesprice">￥{{c.price}}</text>
								<text style="font-size: 16px;" v-else>￥{{c.salesprice}}<text
										style="font-size: 12px;color: #a1a1a1;text-decoration:line-through;">￥{{c.price}}</text></text>
								<view style="display: flex;align-items: center;" v-if="!c.spec">
									<!-- 减号 -->
									<view v-show="c.goods_num>0">
										<u-icon name="minus-circle-fill" color="#FF9A0A" size="25" @click="numJIAN(c)"></u-icon>
									</view>
									<!-- 数量 -->
									<text style="color:#a1a1a1;margin: 0 20rpx;font-size: 16px;"
										v-show="c.goods_num>0">{{c.goods_num}}</text>
									<!-- 加号 -->
									<u-icon name="plus-circle-fill" color="#FF9A0A" size="25" @click="numJIA(c)"></u-icon>
								</view>
								<!-- 规格 -->
		...
		<!-- 规格 -->
		<u-popup :show="show" mode="bottom" overlayOpacity="0.8" :round="10" @close="close" :closeable="true">
			<view style="width: 350px;padding: 10px;">
				<view class="t_title">
					<image :src="url1+food.image" mode=""></image>
					<view class="d_title">
						<u--text color="black" size="15" bold :text="food.name"></u--text>
						<!-- <u--text type="info" size="12" text="产品描述"></u--text> -->
					</view>
				</view>
				<u-line color="#ddd"></u-line>
				<view class="sku-list" v-for="item in userChooseSkuList" :key="item.id">
					<!-- sku标题 -->
					<view class="sku-title">{{item.skuTitle}}</view>
					<!-- 供用户选择的skuLabel -->
					<view class="sku-label-list">
						<view class="label" v-for="(label,index) in item.skuList" @tap="skuLabelClick(item,label,index)"
				...
				<view class="abc">
					已选规格：<text v-for="(item,index) in userChooseSku" :key="index">{{item}}.</text>
				</view>
				<view class="money">
					<text>￥{{rule_price}} 库存:{{rule_store_count}}</text>
					<u-number-box v-model="value">
						<view slot="minus" class="minus">
							<u-icon name="minus" size="12"></u-icon>
						</view>
						<text slot="input" style="text-align: center;" class="input">{{value}}</text>
						<view slot="plus" class="plus">
							<u-icon name="plus" color="#FFFFFF" size="12"></u-icon>
						</view>
					</u-number-box>
				</view>
				<u-button shape="circle" @click="add" color="#00d22a" text="加入购物车"></u-button>
			</view>
		</u-popup>
		<!-- 商家 -->
		...
		<view v-show="hidden" class="box">
				...
				<view class="box_2" v-for="(item,e) in gouwucheAdd" :key="e" v-if="item.goods_num">
					...
					<view class="box_3">
						...
						<view style="color: #a1a1a1;font-size: 20rpx;font-weight: lighter;">
							<text v-for="(i,k) in item.ChooseSku" :key="k">{{i}}.</text>
						</view>
						<view style="display: flex;width: 100%;align-items: center;justify-content: space-between;">
							<text style="color: red;font-size: 16px;" v-if="!item.salesprice">￥{{item.price}}</text>
							<text style="color: red;font-size: 16px;" v-else>￥{{item.salesprice}}
								<text
									style="font-size: 12px;color: #a1a1a1;text-decoration:line-through;">￥{{item.price}}</text>
							</text>
							<view style="display: flex;align-items: center;">
								<u-icon name="minus-circle-fill" color="#FF9A0A" size="25" @click="numJIAN(item)"></u-icon>
								<view style="margin: 0 20rpx;" v-show="item.goods_num>0">{{item.goods_num}}</view>
								<u-icon name="plus-circle-fill" color="#FF9A0A" size="25" @click="numJIA(item)"></u-icon>
							</view>
						</view>
					</view>
			...
			<view class="box_name_price">
				<view>{{shanjia.box_name}}</view>
				<view class="box_price">￥{{shanjia.box_price}}</view>
			</view>
		...
		<view class="bottom">
			<view class="vie5" :style="{display: `${sjStatic}`}">
				<view style="position: relative;top: 0;left: -24rpx;width: 23%;">
				...
				<view style="display: flex;align-items: center;width: 77%;justify-content: flex-start;width: 80%">
					<view
						style="color: #FFFFFF; font-size: 38rpx;border-right: 1px solid white;margin-right: 20rpx;padding-right: 20rpx;">
						{{totalPrice==shanjia.box_price?'￥0':'￥'+totalPrice}}
					</view>
					<view style="color: #FFFFFF;font-size: 24rpx;">
						另需配送费￥{{shanjia.delivery_price}}
					</view>
				</view>
				<block v-if="shanjia.status==1">
					<view :class="lack<=0?'vie24':'vie12'" @click="lack<=0?submit():''">
						{{lack<=0?'去结算':'￥'+lack*100/100+'起送'}}
					</view>
				</block>
...
<script>
	export default {
		...
		onReady() {
			setTimeout(() => {
				var dabaofeiStatic = this.dabaofeiStatic

				function fun(a) {
					return dabaofeiStatic
				}
				let _that = this;
				uni.getSystemInfo({
					...
					shop_details.forEach(el => {
						goods_id.forEach(el2 => {
							if (el.goods_id == el2) {
								el.goods_num = 1; //数量
								el.isselect = true; //选中
								this.total += (el.salesprice != '' ? el.salesprice : el.price) * el
									.goods_num //此处修改页面的价格
								el.total += (el.salesprice != '' ? el.salesprice : el.price) * el
									.goods_num //此处修改自定义商品的价格
		...
		computed: {
			...
			lack() {
				if (this.shop_details.length !== 0) { //必须加这条判断，否则会直接拿data里的数组
					let lack = 0; //此处声明变量，data无需声明，会报错
					let sum = [];
					var total = 0;
					this.gouwucheAdd.forEach(el => {
						if (!el.salesprice) {
		...
		methods: {
			...
			specifications(c, e) {
				...
				guige.forEach(el => {
					...
					for (let i = 0; i < el.name.length; i++) {
						arr.push(el.name[i].item)
					}
					rule[el.spec_name] = arr
				})
				// console.log(rule)
				...
				for (let item in rule) {
					...
					// 这里组织sku列表
					for (let j = 0; j < rule[item].length; j++) {
						// 把接口返回的列表skuHttpList里面的值放到skuObj的skuList里面
						skuObj.skuList.push({
							id: skuLabelId + j,
							checked: false,
							labelText: rule[item][j]
						})
					}
				...
				uni.$u.http.post('api/stores/getsku', {
					...
					let service = [];
					res.data.data.forEach(el1 => {
						var productSpecs = {};
						guige.forEach((el, index) => {
							productSpecs[el.spec_name] = el1.specname[index]
						})
						this.productSpecs.push(productSpecs)
					})

					res.data.data.map((i, k) => {
						i.specname = this.productSpecs[k]
					})
			...
			skuLabelClick(skuRow, skuLabel, index) {
				...
				this.list.forEach(item => {
					// 调用对象比较的方法，确定是哪个sku
					if (this.isObjectValueEqualNew(this.userChooseSku, item.specname)) {
						var arr = Object.keys(item.specname);
						var arr1 = Object.keys(this.userChooseSku);
						// 这里的item就是匹配到的值了
						// console.log(item)
						// 判断规格是否已选择完,选完赋值价格以及数量
						if (arr.length == arr1.length) {
							this.rule_price = item.price
							this.rule_store_count = item.store_count
			...
			isObjectValueEqualNew(a, b) {
				var o1keys = Object.getOwnPropertyNames(JSON.parse(JSON.stringify(a)))
				for (var i = 0; i < o1keys.length; i++) {
					var propName = o1keys[i]
			...
			// 把所选规格参数加入购物车
			add() {
				if (Object.keys(this.userChooseSku).length < this.list.length) {
					uni.showToast({
						title: '请选择规格',
						icon: "none"
					})
				} else {
					if (this.rule_price == 0) {
						uni.showToast({
							title: '规格不存在,重新选择',
							icon: "none"
						})
					} else if (this.rule_store_count == 0) {
						uni.showToast({
							title: '库存不足,重新选择',
							icon: "none"
						})
						...
						this.rule_price = 0
						this.rule_store_count = 0
						this.show = false
						this.userChooseSkuList = [] //清除规格
						this.shop_details[this.e].goods_num = this.shop_details[this.e].goods_num + 1; // 数量
						this.gouwucheYuandian = true; // // 购物车的小圆点出现
						this.gouwuImg = '../../static/19.svg'; //换绿色购物车
						
						this.id.push(this.SelectRule.id)
						var setList = new Set(this.id)
						this.goods_id[this.shop_details[this.e].goods_id] = setList
						console.log(this.goods_id)
						
						//往购物车里添加商品
						this.gouwucheAdd.push({
							goods_id: this.shop_details[this.e].goods_id,
							image: this.shop_details[this.e].image,
							name: this.shop_details[this.e].name,
							price: this.SelectRule.price,
			...
			numJIA(c) {
				...
				for (var h = 0; h < this.gouwucheAdd.length; h++) {
					if (this.gouwucheAdd[h].goods_id === c.goods_id && this.gouwucheAdd[h].ChooseSku === c.ChooseSku) {
