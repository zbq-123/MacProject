<template>
	<view class="page">
		<u-navbar title="闲置物品" :autoBack="true" bgColor="transparent" leftIcon="arrow-left" :fixed="true"
			...
			<swiper :indicator-dots="true" :autoplay="true" :duration="500" :circular="true" indicator-color="#fff">
				...
				<swiper-item v-for="(item, i) in swiperList" :key="i">
					<view class="swiper-item">
						<image :src="url1+item.picture" mode="widthFix"></image>
					</view>
				</swiper-item>
			</swiper>
			<!-- 分类 -->
			<view class="comment">
				<view class="model" v-for="(baseListItem,index) in baseList" :key="index"
					@click="spot(baseListItem,index)">
					<image :src="url1+baseListItem.image" mode="widthFix"></image>
...
<script>
	export default {
		...
		methods: {
			...
			goods() {
				//闲置商品列表
				uni.$u.http.post('api/idlegoods/get_idle_goods', {}).then(res => {
					this.list = res.data.data
				}).catch(err => {
					console.log('400', err)
					uni.showToast({
